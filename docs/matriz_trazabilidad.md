...
| µ(k,a), η(k,a), Σ(k,a) desde la ontología (canal Cronos derivado; Atlas declarado) | Def. 11.1 + Prop. 11.2 + Gea 9.3 (+ Atlas 9.4-9.5, pendiente del frente 3) | `cosmology/mu_eta_cronos.py` + `cosmology/extended_likelihoods.py` | `tests/test_mu_eta_cronos.py` + `tests/test_mu_eta_lock.py` | derivación interna con cierres declarados (E8); predicción sin contraste — no detección; amplitud = límite superior; colas de Atlas pendientes (frente 3) |
| Canal Atlas de µ/η derivado desde la Acción de Gea (frente 3) | Acción de Gea (9.1) + Término de Cronos (9.4); Sello (9.3); ventana (9.4); H.2.2 | `cosmology/mu_eta_atlas.py` + `validation/atlas_derivation.py` + `validation/atlas_tail_derivation.py` | `tests/test_mu_eta_atlas.py` + `tests/test_mu_eta_atlas_lock.py` + `tests/test_mu_eta_atlas_tail.py` + `tests/test_mu_eta_atlas_tail_lock.py` | derivación desde la acción verificada por cuatro vías (GR, Sello, cruce khronométrico, integración completa a 5e-4) — interna (E8); colas — residuos del polo derivados (E3a = A), partes regulares numéricas y confirmación numérica pendiente (E3b = C, abierta); PPN pendiente |
...
| Contraste de los Residuos vía BBN — G_cosmo/G_N libre sobre Y_P (EMPRESS XV) + D/H (14-sep-2026) | Sello de Newton (9.3) + frente 3 (#18/#20): G_cosmo/G_local = (2ξ−α_a)/(3λ_K−1); BBN ve G solo en H | `cosmology/bbn_g.py` + `validation/bbn_response.py` | `tests/test_bbn_g.py` + `tests/test_bbn_g_lock.py` | contraste interno bajo preinscripción (E8) sobre valores publicados transcritos — desenlace A provisional por procedencia; no detección, no demostración física |
